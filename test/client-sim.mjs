...
const sandbox = {
  ...
  navigator: { mediaDevices: null, clipboard: null, userAgent: 'node-test', platform: 'test', maxTouchPoints: 0 },
  matchMedia: () => ({ matches: false, addEventListener(){}, removeEventListener(){}, addListener(){}, removeListener(){} }),
  screen: { orientation: null, width: 1280, height: 720 },
...
section('스냅샷 버퍼 보간');
{
  ...
  const p2 = P.p2;
  // 80ms 간격 스냅 두 개
  p2.hist = [{ t: 1000, x: 100, y: 200 }, { t: 1080, x: 180, y: 200 }];
  Game.interpOthers(1040);                       // 정확히 중간 시점
  ok('두 스냅 사이 중간 위치로 보간', Math.abs(p2.rx - 140) < 0.01, p2.rx);
  Game.interpOthers(1200);                       // 버퍼 끝을 지남
  ok('버퍼 끝에서 외삽하지 않고 정지', p2.rx === 180, p2.rx);
  Game.interpOthers(900);                        // 버퍼 시작 전
  ok('버퍼 시작 전에는 첫 위치', p2.rx === 100, p2.rx);
}
