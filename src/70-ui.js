...
· 토론 시간이 기본 60초로 길게 잡혀 있습니다. 방장이 설정에서 늘릴 수 있습니다.<br>
  ...
  revealRole() {
    ...
    const todo = G.hunt ? (r.faction === F.DUCK ? [
      ...
      ['🎭', '<b>혼자만의 승리 조건</b>이 있습니다 — 위 설명을 확인하세요'],
      ['🗺️', '양도 늑대도 아닙니다. 상황에 따라 편을 고르세요'],
      ['💬', '회의에서 정체를 들키면 추방당하기 쉽습니다'],
    ] : [
      ['📋', '<b>노란 표시</b>를 따라가 <b>✋ 사용</b> — 임무를 하면 승리에 가까워집니다'],
      ['📢', '<b>시체를 발견하면 신고</b> — 즉시 전원 회의가 열립니다'],
    ...
    root.innerHTML = `
      <div style="display:flex;gap:16px;align-items:center;max-width:560px;margin:0 auto">
        <div style="flex:none;width:118px;text-align:center">
          <div style="font-size:46px;line-height:1;margin-bottom:4px">${r.icon}</div>
          <div style="font-size:12px;color:${col};font-weight:800">${FACTION_LABEL[r.faction]} 진영</div>
    ...
    if (G.ducksKnown.length > 1) {
      const names = G.ducksKnown.filter(i => i !== G.myId).map(i => G.players[i]?.name).filter(Boolean);
      if (names.length) root.appendChild(h('div', { style:{ marginTop:'10px', padding:'9px', background:'#2a1b28', borderRadius:'12px', border:'1px solid #63384f' } },
        h('div', { cls:'tiny dim' }, '동료 늑대'), h('div', { style:{ fontWeight:800, color:'var(--duck)' } }, names.join(', '))));
    }
    const btn = h('button', { cls:'btn primary', style:{ width:'100%' }, onclick: () => {
      localStorage.setItem('duckus_played', '1');
      UI.closeModal();
    } }, '시작하기');
    this.modal({ title: firstTime ? '내 역할 (나에게만 보입니다)' : '역할이 정해졌습니다', body: root, footer:[btn], closable:false });
    // 처음 하는 사람은 읽을 시간이 필요하다 — 자동 닫힘을 늦춘다
    setTimeout(() => { if (UI.modalStack.length) UI.closeModal(); }, firstTime ? 20000 : 9000);
  },
