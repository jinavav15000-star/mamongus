...
/* ─── 5. 소행성 격추 (시각 임무) ─────────────────────────────────────────*/
asteroid: {
  title: '소행성 12개를 격추하세요',
  build(root, opt, done) {
    const cv = h('canvas', { cls:'mg-canvas' });
    const hud = h('div', { cls:'mg-msg' }, '남은 소행성 12');
    root.append(cv, hud);
    ...
    cv.addEventListener('pointerdown', e => {
      ...
      for (let i = rocks.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(k.x - x, k.y - y) < k.r + 16) {
          ...
          hud.textContent = '남은 소행성 ' + Math.max(0, 12 - hits);
    ...
    (function loop() {
      ...
      g.fillStyle = '#05070f'; g.fillRect(0, 0, w, hgt);
      for (let i = 0; i < 40; i++) { g.fillStyle = 'rgba(255,255,255,.25)'; const sx = (i * 97) % w, sy = (i * 61 + (performance.now()/40)) % hgt; g.fillRect(sx, sy, 1.5, 1.5); }
      ...
      for (const k of rocks) {
        ...
        g.fillStyle = '#6b7280'; g.strokeStyle = '#9aa3af'; g.lineWidth = 2;
        g.beginPath();
        for (let s = 0; s < 8; s++) { const a = s / 8 * 6.283, rr = k.r * (0.78 + ((s * 37) % 10) / 40); g.lineTo(Math.cos(a) * rr, Math.sin(a) * rr); }
        g.closePath(); g.fill(); g.stroke(); g.restore();
      }
      // 함선 + 레이저
      g.fillStyle = '#5fd0ff'; g.beginPath(); g.moveTo(w/2, hgt - 26); g.lineTo(w/2 - 13, hgt - 4); g.lineTo(w/2 + 13, hgt - 4); g.closePath(); g.fill();
      for (let i = shots.length - 1; i >= 0; i--) { const s = shots[i]; s.t += .16;
        g.strokeStyle = `rgba(255,90,90,${1 - s.t})`; g.lineWidth = 3; g.beginPath(); g.moveTo(s.x, s.y); g.lineTo(s.x + (s.tx - s.x) * Math.min(1, s.t * 2), s.y + (s.ty - s.y) * Math.min(1, s.t * 2)); g.stroke();
...
download: {
  ...
  build(root, opt, done) {
    ...
    const label = h('div', { cls:'mg-dl-label' }, opt.up ? '데이터 업로드' : '데이터 다운로드');
    const bar = h('div', { cls:'mg-bar' }); const fill = h('div', { cls:'mg-bar-fill' }); bar.appendChild(fill);
    const pct = h('div', { cls:'mg-msg' }, '대기 중');
    const btn = h('button', { cls:'mg-bigbtn' }, opt.up ? '⬆ 업로드 시작' : '⬇ 다운로드 시작');
...
sample: {
  ...
  build(root, opt, done) {
    ...
    const btn = h('button', { cls:'mg-bigbtn' }, '🧪 분석 시작');
    ...
    btn.addEventListener('click', () => {
      ...
      timer = setInterval(() => {
        ...
        if (t >= 25) { clearInterval(timer); phase = 2; tubes.forEach(tb => tb.classList.remove('scan'));
          tubes[target].classList.add('anomaly'); msg.textContent = '이상 샘플을 선택하세요'; btn.textContent = '분석 완료'; }
      }, 130);
    });
...
chart: {
  ...
  build(root, opt, done) {
    ...
    (function loop() {
      ...
      g.strokeStyle = 'rgba(95,208,255,.28)'; g.lineWidth = 2; g.setLineDash([7, 7]);
      ...
      pts.forEach((p, i) => { g.beginPath(); g.arc(p.x, p.y, 13, 0, 6.283);
        g.fillStyle = i < idx ? '#2ea44f' : (i === idx ? '#ffd23d' : 'rgba(255,255,255,.18)'); g.fill(); });
      g.save(); g.translate(ship.x, ship.y); g.fillStyle = '#5fd0ff';
      g.beginPath(); g.moveTo(0, -13); g.lineTo(10, 11); g.lineTo(-10, 11); g.closePath(); g.fill(); g.restore();
      raf = requestAnimationFrame(loop);
    })();
    root._cleanup = () => { cancelAnimationFrame(raf); window.removeEventListener('resize', resize); };
  },
...
sort: {
  ...
  build(root, opt, done) {
    ...
    let left = 6;
    const msg = h('div', { cls:'mg-msg' }, '남은 화물 6'); root.appendChild(msg);
    for (let i = 0; i < 6; i++) {
      const ci = i % 3;
      ...
      onDrag(bx, {
        ...
        move(p, e) { const b = area.getBoundingClientRect(); bx.style.left = (e.clientX - b.left - ox) + 'px'; bx.style.top = (e.clientY - b.top - oy) + 'px'; },
          ...
          if (e.clientX > t.left && e.clientX < t.right && e.clientY > t.top && e.clientY < t.bottom) {
            bx.remove(); left--; msg.textContent = '남은 화물 ' + left; Sfx.taskStep();
...
shields: {
  ...
  build(root, opt, done) {
    ...
    let left = lit.size;
    const msg = h('div', { cls:'mg-msg' }, `남은 조각 ${left}`); root.appendChild(msg);
...
scan: {
  ...
  build(root, opt, done) {
    ...
    btn.addEventListener('click', () => {
      if (running) return; running = true; btn.disabled = true; btn.textContent = '스캔 중…';
      t0 = performance.now();
      (function loop() {
